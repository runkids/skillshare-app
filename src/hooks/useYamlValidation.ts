...
export function validateYaml(source: string, validTargets: string[]): ValidationError[] {
    ...
    for (const [name, cfg] of Object.entries(parsed.targets)) {
      if (cfg && typeof cfg === 'object' && 'mode' in cfg) {
        validateEnum(
          errors,
          sourceLines,
          (cfg as Record<string, unknown>).mode,
          'mode',
          VALID_SYNC_MODES,
          `mode for target "${name}"`,
          name
        );
      }
    }
  ...
  if (parsed.audit && typeof parsed.audit === 'object') {
    ...
    validateEnum(
      errors,
      sourceLines,
      parsed.audit.profile,
      'profile',
      VALID_AUDIT_PROFILES,
      'audit profile',
      'audit'
    );
    validateEnum(
